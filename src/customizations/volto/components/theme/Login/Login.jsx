...
// Import authomatic components
import {
  authomaticRedirect,
  listAuthOptions,
  oidcRedirect,
} from '@plone-collective/volto-authomatic/src/actions';
import AuthProviders from '@plone-collective/volto-authomatic/src/components/AuthProviders/AuthProviders';
...
const Login = (props) => {
  ...
  const authLoading = useSelector((state) => state.authOptions.loading);
  ...
  // Prepare tabs
  const validProviders = authOptions
    ? authOptions.filter((provider) => provider.id !== 'oidc')
    : [];
            ...
            render: () => (
              <Tab.Pane>
                <Segment className="form">
                  ...
                  {(authLoading || validProviders.length === 0) && (
